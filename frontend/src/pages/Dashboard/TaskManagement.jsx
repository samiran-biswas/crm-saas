...
import {
  Button, Card, Tag, Modal, Input, Select, Form, Row, Col, Tooltip, DatePicker, Spin, Table, Tabs, Calendar
} from 'antd';
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="task-management-container p-6 space-y-4">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-semibold text-blue-600">Task Management</h1>
...
const TaskCard = ({ task, moveTask, employees, onEdit }) => {
  ...
  return (
    ...
    >
      ...
      <div>
        <span>Assigned to: {task.assignedTo.map(id => employees.find(e => e.id === id)?.name).join(', ')}</span>
      </div>
      {task.dueDate && (
        <div>
